...
module.exports = function (cassandraClient) {
  ...
  router.get('/api/v1/url', async function (req, res, next) {
    ...
    if (!originalUrl) {
      return res.send(responses.errResponse(false, "You must provide a url to shorten"));
    }

    const shortUrl = `${baseURL}/${letters}`;
    const requesterIp = req.ip;

    res.send(responses.dataResponse({
      url: shortUrl
    }));
    ...
    cassandraClient.execute(statements.INSERT_URL, [originalUrl, shortUrl, LocalDate.now(), requesterIp]);
  });
